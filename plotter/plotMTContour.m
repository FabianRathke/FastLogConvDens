function plotMTContour(X,gridX,gridY,estimate,k,XLim,YLim,folder,filename,contourLines)

if ~exist('contourLines','var')
	contourLines = exp([-2.1:-0.5:-40]);
end

figure('visible','on'); [C h] = contour(gridX,gridY,estimate,contourLines); hold on; %clabel(C,h);
