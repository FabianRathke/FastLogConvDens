function [params logLike statistics] = newtonBFGSLCInit(params,X,gamma,gridParams,options)
% *** TODO WRITE HELP ***
totTime = tic;
if nargin < 5
	options = struct();
...
%tic; [gradA gradB TermA TermB] = calcGradFull(X,gridParams.grid(1:dim),a,b,gamma,gridParams.weight,gridParams.delta,influence,sW,gridParams.gridSize,gridParams.YIdx);  grad = gradA + gradB; timing.evalGrid = timing.evalGrid + toc;
tic; [gradA gradB TermA TermB] = calcGradFloatAVX(single(X),single(gridParams.grid(1:dim)),single(a)',single(b),gamma,gridParams.weight,single(gridParams.delta),influence,single(sW),gridParams.gridSize,gridParams.YIdx); grad = double(gradA+gradB); timing.evalGrid=timing.evalGrid + toc;
...
%	tic; [gradA gradB TermA TermB] = calcGradFull(X,gridParams.grid(1:dim),aNew,bNew,gamma,gridParams.weight,gridParams.delta,influence,sW,gridParams.gridSize,gridParams.YIdx);  grad = gradA + gradB; timing.evalGrid = timing.evalGrid + toc;
	tic; [gradA gradB TermA TermB] = calcGradFloatAVX(single(X),single(gridParams.grid(1:dim)),single(aNew)',single(bNew),gamma,gridParams.weight,single(gridParams.delta),influence,single(sW),gridParams.gridSize,gridParams.YIdx); grad = double(gradA+gradB); timing.evalGrid=timing.evalGrid + toc;
