...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	double *s_k = (double *) mxGetData(prhs[0]);
   	double *y_k = (double *) mxGetData(prhs[1]);
	double *sy = (double *) mxGetData(prhs[2]);
    double *syInv = (double *) mxGetData(prhs[3]);
    double step = (double)  mxGetScalar(prhs[4]);
    double *grad = (double *) mxGetData(prhs[5]);
    double *gradOld = (double *) mxGetData(prhs[6]);
   	double *newtonStep = (double *) mxGetData(prhs[7]);
    ...
    const int dimsA[] = {nH};
	plhs[0] = mxCreateNumericArray(1,dimsA,mxDOUBLE_CLASS,mxREAL);
	double* newtonStepNew = (double *) mxGetData(plhs[0]);
...
/*		tmp = 0;
		...
		{
			__m256d tmp_;
			tmp_ = _mm256_set1_pd(0);
			#pragma omp for
			for (j=0; j < nH-nH%4; j+=4) {
				tmp_ = _mm256_fmadd_pd(_mm256_loadu_pd(q+j),_mm256_loadu_pd(s_k+curCol*nH + j),tmp_);
			}
			double* t = (double *) &tmp_;
			#pragma omp critical
			tmp += t[0] + t[1] + t[2] + t[3];
		}*/
		tmp = 0;
