	...
	#pragma omp parallel num_threads(NUMCORES)
    {
		...
		for (j=0; j < N; j++) {
            ...
            for (i=0; i < numElements; i++) {
                ftTmp = bGamma[elementList[i+basicIdx]] + aGamma[elementList[i+basicIdx]]*X[j];
	            for (k=1; k < dim; k++) {
                    ftTmp += aGamma[elementList[i+basicIdx]+k*nH]*X[j + (k*N)];
                }
			...
			for (i=0; i < numElementsExp; i++) {
				...
				ftTmp = sum_ft_inv*ft[i];
				for (k=0; k < dim; k++) {
					grad_ft_private[idxSave + (k*nH)] += ftTmp*X[j+(k*N)];
				}
				grad_ft_private[idxSave + (dim*nH)] += ftTmp;
			}
		}
    	#pragma omp critical
        {
